...
export default function ProfilePage() {
  ...
  return (
    <div>
      {profile.isLoading || (userReview.isLoading && <Loader />)}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-8 gap-4">
          ...
          <div className="lg:col-span-5 flex flex-col gap-4">
            {isProfileModificationOpend ? (
              <ProfileMofication onclick={isOpenProfile} />
            ) : (
              <div className="lg:col-span-5 flex flex-col gap-4">
                <ProfileDescription />
                <OwnerTools />
                {/* i'll change this this compoenent to UserReviews, add it to UserReview Folder. */}
                <ToolReviews reviews={[]} />
              </div>
            )}
          </div>
