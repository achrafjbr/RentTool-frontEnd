...
import { renteTool } from "../../../rental/rentalThunks";
...
export default function ToolReservation({ tool }: { tool?: Tool }) {
  ...
  const { isLoading, error } = useAppSelector((state) => state.renter);
          ...
          >
            ...
            <ToolButton
              onclick={async () => {
                if (
                  toolReservation.startDate == "" ||
                  toolReservation.endDate == "" ||
                  toolReservation.startDate > toolReservation.endDate
                ) {
                  toast.error("La date de reservation n'est pas convenable ");
                  return;
                }
                await dispatch(
                  renteTool({
                    tool: tool!._id!,
                    startDate: toolReservation.startDate,
                    endDate: toolReservation.endDate,
                  }),
                );
              }}
              prefix={true}
