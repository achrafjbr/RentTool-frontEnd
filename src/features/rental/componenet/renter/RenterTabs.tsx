...
export default function RenterTabs() {
  const { activeRentals, totalRequests } = useAppSelector(
    (state) => state.renter,
  );
  ...
  const renterTabs = [
    {
      ...
      label: `locations actives (${activeRentals.length})`,
    },
    { name: MY_REQUESTS, label: `Toutes mes demandes (${totalRequests})` },
  ];
  ...
  return (
    <div>
      ...
      >
        {renterTabs.map(({ label, name }) => (
          <p
            ...
            className={`text-xs font-semibold capitalize px-8 py-2.5
      ...
      <div className="text-gray-600 ">
        {tabActive === MY_REQUESTS ? <RentalRequests /> : <ActiveRentals />}
      </div>
    </div>
  );
}
